...
# After the above command runs, there will be a folder named install-tl in folder /usr/local/src
  ...
  && ./install-tl -repository http://mirror.utexas.edu/ctan/systems/texlive/tlnet -profile profile && cd .. \
  && rm -rf install-tl
...
RUN git clone https://github.com/brucemiller/LaTeXML.git && cd LaTeXML && perl Makefile.PL && make \
